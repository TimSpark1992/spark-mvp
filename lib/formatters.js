...
export const formatPrice = (priceCents, currency = 'USD') => {
  ...
  console.log('formatPrice called with:', { priceCents, currency, type: typeof priceCents })
  ...
  if (priceCents === null || priceCents === undefined || isNaN(priceCents) || priceCents < 0) {
    console.log('formatPrice: returning default $0.00 due to invalid value')
  ...
  const currencyInfo = CURRENCIES.find(c => c.code === currency)
  const formattedPrice = `${currencyInfo?.symbol || '$'}${price.toFixed(2)}`
  console.log('formatPrice: returning', formattedPrice)
