...
if (isBuildTime) {
  console.warn('⚠️ SUPABASE BUILD MODE:')
  console.warn('Environment variables not available during build - using fallback configuration')
  ...
  console.warn('- SUPABASE_SERVICE_ROLE_KEY:', supabaseServiceRoleKey ? '✅ Present' : '❌ Missing')
} else {
...
// Configure Supabase client with timeout handling and proper session persistence
export const supabase = isBuildTime ? null : createClient(supabaseUrl, supabaseAnonKey, {
  ...
  global: {
    fetch: (url, options = {}) => {
      ...
      // Set 50-second timeout for Supabase operations (longer than frontend 45s timeout to prevent race conditions)
...
export const createRateCard = async (rateCardData) => {
  try {
    console.log('🔄 Creating rate card with data:', rateCardData)
    ...
    if (error) {
      console.error('❌ Supabase createRateCard error:', error)
      return { data: null, error }
    }
    
    // Return the first item from the array (similar to campaigns fix)
    const rateCard = Array.isArray(data) ? data[0] : data
    console.log('✅ Rate card created successfully:', rateCard?.id)
    
    return { data: rateCard, error: null }
  } catch (err) {
    console.error('❌ Exception in createRateCard:', err)
    return { data: null, error: err }
  }
}
...
export const updateRateCard = async (rateCardId, updates) => {
  try {
    console.log('🔄 Updating rate card:', rateCardId, updates)
    
    // Use admin client to bypass RLS policies
    const client = supabaseAdmin || supabase
    console.log('Using client:', client === supabaseAdmin ? 'Admin (Service Role)' : 'Standard (Anonymous)')
    ...
    if (error) {
      console.error('❌ Supabase updateRateCard error:', error)
      return { data: null, error }
    }
    
    // Return the first item from the array (consistent with other functions)
    const rateCard = Array.isArray(data) ? data[0] : data
    console.log('✅ Rate card updated successfully:', rateCard?.id)
    
    return { data: rateCard, error: null }
  } catch (err) {
    console.error('❌ Exception in updateRateCard:', err)
    return { data: null, error: err }
  }
}
...
export const deleteRateCard = async (rateCardId) => {
  try {
    console.log('🔄 Deleting rate card:', rateCardId)
    ...
    if (error) {
      console.error('❌ Supabase deleteRateCard error:', error)
      return { data: null, error }
    }
    
    // Return the first item from the array (consistent with other functions)
    const rateCard = Array.isArray(data) ? data[0] : data
    console.log('✅ Rate card deleted successfully:', rateCard?.id)
    
    return { data: rateCard, error: null }
  } catch (err) {
    console.error('❌ Exception in deleteRateCard:', err)
    return { data: null, error: err }
  }
}
...
// File upload helpers with bucket verification
export const uploadFile = async (bucket, path, file) => {
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes('bucket') || error.message?.includes('not found')) {
        return { 
          data: null, 
          error: { 
            message: `Storage bucket '${bucket}' is not configured. Please contact support to set up file uploads.`,
    ...
    console.error(`❌ Upload exception for bucket ${bucket}:`, err)
    return { 
      data: null, 
      error: { 
        message: 'Upload failed due to connection or configuration issue',
        code: 'UPLOAD_FAILED'
      }
    }
  }
...
export const getFileUrl = (bucket, path) => {
  try {
    console.log(`🔗 Getting URL for bucket: ${bucket}, path: ${path}`)
    
    const { data } = supabase.storage
      .from(bucket)
      .getPublicUrl(path)
    
    console.log(`✅ URL generated for ${bucket}:`, data?.publicUrl)
    return data?.publicUrl
  } catch (err) {
    console.error(`❌ URL generation failed for bucket ${bucket}:`, err)
    return null
  }
}
