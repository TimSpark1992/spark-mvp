...
export const FIELD_SANITIZERS = {
  ...
  description: (value) => {
    // Special handling for description to preserve spaces and line breaks  
    if (!value || typeof value !== 'string') return ''
  ...
  creator_requirements: (value) => {
    // Special handling for creator requirements to preserve spaces
    if (!value || typeof value !== 'string') return ''
  ...
  // Application fields
  note: (value) => {
    // Special handling for notes to preserve spaces
    if (!value || typeof value !== 'string') return ''
