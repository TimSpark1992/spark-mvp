...
export const authSignUp = async (formData) => {
  try {
    // Validate input data with enhanced XSS protection
    ...
    // If user was created, create profile with sanitized data
    if (data.user) {
      const profileData = {
        id: data.user.id,
        email: email, // Already sanitized by validation
        full_name: fullName, // Already sanitized by validation
        role: role
      }
