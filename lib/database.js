...
import { sanitizeObject, validateInput, campaignSchema, applicationSchema, profileUpdateSchema } from './validation'
...
export const dbUpdateProfile = async (userId, updates, requestingUserId, requestingUserRole) => {
  try {
    ...
    const sanitizedUpdates = sanitizeObject(validation.data)
...
export const dbCreateCampaign = async (campaignData, brandId, requestingUserRole) => {
  try {
    ...
    const validation = validateInput(campaignSchema, campaignData)
    if (!validation.success) {
      return { data: null, error: { message: validation.error } }
    }
