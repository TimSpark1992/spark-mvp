...
import { formatPrice as utilFormatPrice, formatDate } from '@/lib/formatters'
...
/**
 * Format price in cents to currency string - Use centralized formatter
 */
...
/**
 * Validate offer pricing - FIXED validation logic
...
export const validateOfferPricing = (offerData) => {
  ...
  if (offerData.items) {
    offerData.items.forEach((item, index) => {
      ...
      // FIXED: Use specific null/undefined checks instead of !item.unit_price_cents
      if (item.unit_price_cents === null || item.unit_price_cents === undefined || 
          isNaN(item.unit_price_cents) || item.unit_price_cents <= 0) {
        errors.push(`Item ${index + 1}: Valid unit price is required`)
      }
      
      // FIXED: Use specific null/undefined checks instead of !item.qty
